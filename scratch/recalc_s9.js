...
const multiDbMatch = currentHtml.match(/MULTI_DB = (\{[\s\S]*?season[89]: \{[\s\S]*?\}\s*\}\s*);/);
if (!multiDbMatch) {
    ...
    if (inn && inn.bat) {
      inn.bat.forEach((b, idx) => {
        ...
        if (!playerPositions[name]) {
          playerPositions[name] = { count: 0, positions: {} };
        }
...
console.log("--- TOP BATTERS BY POSITION ---");
for (let pos = 1; pos <= 8; pos++) {
  let playersAtPos = sortedBatting.filter(b => b.mostCommonPos === pos);
  console.log(`\nPosition ${pos}:`);
  playersAtPos.slice(0, 3).forEach(b => {
    console.log(`  ${b.name} (${b.team}): ${b.runs} runs, ${b.avg.toFixed(2)} avg, ${b.sr ? b.sr.toFixed(2) : 0} sr`);
  });
}

console.log("\n--- WORST BATTERS BY POSITION (min 4 inns) ---");
